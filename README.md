...
`pnpm test` 覆盖：两种回答依据的精确系统指令、三种关系上下文、引用排序与去重、问题编辑改道、项目隔离、流式 UI 批量提交、IndexedDB v3 保存恢复、行为信号聚合、提案上限与冷却、无损包往返，以及上游 SSE 到统一事件的转换和错误映射。`pnpm test:e2e` 显式启动本机假模型，跑桌面流式卡片、停止后恢复、跨项目后台与并行生成、390px 无横向溢出，以及“跨日信号 → 只查看幽灵分支 → 编辑问题 → 明确物化”流程；真实 CozAI 请求不在默认测试中执行，避免消耗额度。
