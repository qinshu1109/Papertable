...
- **只读笔记 Harness Alpha**：Web 可建立独立的只读资料库，桌面版可连接已有 Vault；资料库不会变成卡片或污染关系图，项目必须显式绑定后才可检索。模型只得到 `search_notes(query, limit?)` 与 `read_notes(chunkIds)` 两个只读工具，工具范围由宿主冻结，模型不能传路径、Vault 或范围。
