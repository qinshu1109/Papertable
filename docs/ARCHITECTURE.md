...
`src/lib/agent.ts` 是有界编排层，而不是常驻 Agent。每个 run 持久化轮次、调用、墙钟和 provider 报告 token 四维预算；默认最多 4 个工具轮次、8 次工具调用、120 秒和 32,000 token。同一成功调用重复时先提醒，继续重复则以 `no_progress` 收尾；同一异常调用连续失败两次后拒绝再执行。用户停止会取消模型及未执行工具，并在最后完整步骤边界保留可续跑轨迹。

### 原生工具准入与协议修复

Agent 只有一条执行路径：通过三阶段握手的旗舰模型原生工具状态机。握手分别验证强制工具调用发出、`tool` role 结果回灌和流式 `tool_call` 增量；缓存同时绑定接口、模型、协议适配层版本和网关响应形状，并有默认 24 小时 TTL。
...
能力未知 / 探测失败 / 缓存过期 / 适配层或响应形状变化
  → 同一接口、模型、协议重新探测
  → 未通过则拒绝 Agent 准入
```

运行时空响应或畸形工具协议只在同一模型、同一协议内做分类重试、确定性清洗、要求模型重发、同协议非流式重建和能力重探。修复耗尽进入 `failed/protocol_error`，不会离开原生工具协议。普通无资料库聊天不经过 Agent 准入；`sources-only` 在没有合格材料时仍由本地确定性拒答。
