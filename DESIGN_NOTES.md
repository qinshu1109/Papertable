...
页面组件只经由 `useStore()` 读取数据和触发动作。`StoreProvider` 编排 IndexedDB、纯上下文函数和本机模型接口；视觉组件不直接访问数据库、文件或密钥。

- `src/lib/context.ts`：唯一的上下文拼装来源；「本次上下文」只渲染其结果；
- `src/lib/storage.ts`：Dexie/IndexedDB，流式输出按最多 500 ms 保存；
- `src/lib/provider.ts`：浏览器到本机 SSE 代理；
- `src/lib/formats.ts`：开放格式适配器；
- `ConceptPreview`：按概念和来源版本缓存真实回答；
- `lib/markdown.tsx`：保留 `data-turn-ai` 和 `concept-term` 两个交互锚点。

若未来迁移到 Tauri/SQLite，只替换 storage adapter；若替换 Markdown 渲染器，必须保留以上文本选区与概念定位锚点。
